	...
	<table cellspacing="20px" align="center" cellpadding="30px"
		...
		<%
			try {
				...
				String subcategory = request.getParameter("subtype");
				if (type != null ) {
					if(subcategory != null){
						query = "select ItemID,Name,UnitPrice,Photo from inventory where category like '%" + type + "%' and subcategory like '%" + subcategory + "%'";
						db.rs = db.st.executeQuery(query);
					}else{
						query = "select ItemID,Name,UnitPrice,Photo from inventory where category like '%" + type + "%'";
						db.rs = db.st.executeQuery(query);
